...
class EDD_GetUsers_Metas_Endpoint extends WP_REST_Controller
{
    ...
    public function getItems($request)
    {
        ...
        $items = array();
        $items['edd_users_metas'] = $this->retrieveEddUsersMetas($parameters);
        $data = array();
     ...
     * @return Users Meta
     */
    public function retrieveEddUsersMetas($parameters)
    {
        global $wpdb;
        ...
        if ($task == "get_totals") {
            $found_posts = $wpdb->get_var( 
                "SELECT count(`umeta_id`) FROM ".$wpdb->prefix."usermeta" 
            );
            ...
            $users_metas = $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT * FROM 
                    ".$wpdb->prefix."usermeta 
                    WHERE user_id = %d", $post_id 
                )
            );
        } else {
            $users_metas = $wpdb->get_results(
